...
function reactionCount($fb, $objectID, $accessToken) {
  $request = $fb->request('GET', "/?ids={$objectID}&fields=reactions.type(LIKE).limit(0).summary(total_count).as(LIKE),reactions.type(LOVE).limit(0).summary(total_count).as(LOVE),reactions.type(HAHA).limit(0).summary(total_count).as(HAHA),reactions.type(WOW).limit(0).summary(total_count).as(WOW)", [], $accessToken);
...
function drawReactionCount($image, $reactions, $fontSettings) {
  ...
  // Like counter
  $image->text($reactions['LIKE']['count'],
             $reactions['LIKE']['xpos'],
             $reactions['LIKE']['ypos'],
  ...
  // Love counter
  $image->text($reactions['LOVE']['count'],
             $reactions['LOVE']['xpos'],
             $reactions['LOVE']['ypos'],
             function($font) use ($fontSettings) {
               $font->file($fontSettings['FAMILY']);
  ...
  // Haha counter
  $image->text($reactions['HAHA']['count'],
             $reactions['HAHA']['xpos'],
             $reactions['HAHA']['ypos'],
             function($font) use ($fontSettings) {
               $font->file($fontSettings['FAMILY']);
  ...
  // Wow counter
  $image->text($reactions['WOW']['count'],
             $reactions['WOW']['xpos'],
             $reactions['WOW']['ypos'],
             function($font) use ($fontSettings) {
               $font->file($fontSettings['FAMILY']);
